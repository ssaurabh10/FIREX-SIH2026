...
Supports OpenRouter (with multi-key round-robin + reasoning) and offline MockAIProvider.
"""
import re
import json
import logging
...
from app.intelligence.prompts import PROMPT_VERSION
...
class OpenRouterProvider(BaseAIProvider):
    ...
    def investigate(self, prompt: str, image_data_uri: str) -> AIInvestigationReport:
        ...
        while attempts < total_attempts:
            ...
            if not key:
                logger.error("[OpenRouter] No API key available in key pool.")
                break
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    logger.warning(
                        f"[OpenRouter] {key_id} returned HTTP {resp.status_code} ({resp.text[:120]}). Auto-failing over..."
                    )
                    key_pool.rotate_key()
        ...
        try:
            ...
            return AIInvestigationReport(
                ...
                image_quality=ImageQualityAssessment(
                    score=int(img_q_raw.get("score", 80)),
    ...
    def _build_fallback_report(
        ...
        return AIInvestigationReport(
            ...
            visual_evidence=["Optical scene analysis could not be definitively concluded."],
            contextual_evidence=["Telemetry recorded; provider failover triggered."],
            uncertainties=[reason],
            image_quality=ImageQualityAssessment(score=50, cloud_cover="medium", visibility="moderate"),
            needs_reinvestigation=True,
            reasoning_summary=f"Automated classification inconclusive. Reason: {reason}.",
            reasoning_details=raw_text if raw_text else None,
            model_used=self.model,
            key_used=key_id,
            prompt_version=PROMPT_VERSION,
            investigated_at=datetime.utcnow()
...
class MockAIProvider(BaseAIProvider):
    ...
    def investigate(self, prompt: str, image_data_uri: str) -> AIInvestigationReport:
        ...
        return AIInvestigationReport(
            ...
            image_quality=ImageQualityAssessment(score=90, cloud_cover="low", visibility="good"),
