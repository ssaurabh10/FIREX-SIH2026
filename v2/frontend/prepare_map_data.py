...
# Add persistence, risk engine, and selection registry to module search path
_pers_dir = os.path.join(BASE_DIR, "pipeline", "07_persistence")
if _pers_dir not in sys.path:
    sys.path.insert(0, _pers_dir)
import history_db

_sel_dir = os.path.join(BASE_DIR, "pipeline", "02_selection")
if _sel_dir not in sys.path:
    sys.path.insert(0, _sel_dir)
try:
    from facilities_registry import KNOWN_FACILITIES_REGISTRY, find_nearest_facility
except ImportError:
...
def get_indian_location_label(lat: float, lon: float) -> tuple[str, str, str, str, float]:
    """
    Identifies ground facilities and physical classification using verified registry and spatial proximity.
    Returns (location_name, display_name, classification, category_target, distance_km).
    """
    best_match = None
    min_dist = float("inf")
    active_reg = KNOWN_FACILITIES_REGISTRY
    if find_nearest_facility:
        best_match, min_dist = find_nearest_facility(lat, lon, active_reg)
    else:
        for fac in active_reg:
            d = history_db.haversine_km(lat, lon, fac["lat"], fac["lon"])
            if d < min_dist:
                min_dist = d
                best_match = fac

    calc_dist = round(min_dist, 3) if min_dist != float("inf") else 999.0
...
def prepare_data(pass_type=None, progress_cb=None):
    ...
    for c in clusters:
        ...
        loc_label, disp_label, reg_cls, reg_cat, dist_km = get_indian_location_label(lat, lon)
    ...
    for idx, c in enumerate(qualified_clusters, start=1):
        ...
        incidents.append({
            ...
            "latitude": lat,
            "longitude": lon,
            "facility_distance_km": c.get("distance_km"),
            "frp": frp,
