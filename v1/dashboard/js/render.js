...
function factorBullets(cases) {
  ...
  return `<div class="stack-5">${rows.map((r) => {
    ...
    // A suppression row carries a negative score against a ceiling of 0. The
    // guard above is right to refuse the division, but its default of 0 must not
    // then be printed as a measurement: "0% of the weight available" says the
    // factor contributed nothing, on the one row that removes the most.
