...
# Add facilities registry path
_reg_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "02_selection")
if _reg_dir not in sys.path:
    sys.path.insert(0, _reg_dir)
try:
    from facilities_registry import find_nearest_facility, haversine_km, KNOWN_FACILITIES_REGISTRY
except ImportError:
    find_nearest_facility = None
    haversine_km = None
...
def calculate_proximity_score(
    ...
    """
    Evaluates physical geodesic proximity to hazardous petrochemical,
    heavy industrial, mining, or ecological infrastructure using real Haversine distance.
    """
    nearest_fac = None
    if (dist_km is None or dist_km < 0) and lat is not None and lon is not None and find_nearest_facility:
        nearest_fac, dist_km = find_nearest_facility(lat, lon)
    elif dist_km is not None and find_nearest_facility and lat is not None and lon is not None:
        nearest_fac, _ = find_nearest_facility(lat, lon)

    # Clean short facility/location label
    raw_name = nearest_fac["name"] if nearest_fac else location_name
    clean_target = raw_name.split(",")[0].strip() if raw_name else "Strategic Asset"

    # If distance is still unknown
    if dist_km is None or dist_km < 0:
        return 4.0, f"Unverified Ground Distance: {clean_target} (4.0/15 pts)"

    # Identify facility risk profile
    asset_type = nearest_fac.get("asset_type") if nearest_fac else None
    loc_lower = (location_name + " " + (nearest_fac["name"] if nearest_fac else "")).lower()

    if not asset_type:
        if any(k in loc_lower for k in ["refinery", "petrochemical", "ongc", "lng", "chemical", "iocl", "hpcl", "hmel", "bpcl"]):
            asset_type = "petrochemical"
        elif any(k in loc_lower for k in ["steel", "smelter", "power station", "ntpc", "thermal power"]):
            asset_type = "heavy_industry"
        elif any(k in loc_lower for k in ["coal", "mine", "quarry", "seam"]):
            asset_type = "mining"
        elif any(k in loc_lower for k in ["forest", "wildlife", "sanctuary", "canopy", "ridge"]):
            asset_type = "forest_reserve"
        else:
            asset_type = "rural"

    # Real distance decay scoring across asset types
    if asset_type == "petrochemical":
        if dist_km <= 0.5:
            score = 15.0
            bracket = "Critical Petrochemical Battery Limit"
        elif dist_km <= 2.0:
            score = 12.0
            bracket = "Petrochemical Perimeter Buffer"
        elif dist_km <= 5.0:
            score = 8.0
            bracket = "Petrochemical Corridor Vicinity"
        elif dist_km <= 15.0:
            score = 5.0
            bracket = "Regional Industrial Zone"
        else:
            score = 2.0
            bracket = "Remote Petrochemical Buffer"

    elif asset_type in ["heavy_industry", "mining_power"]:
        if dist_km <= 0.5:
            score = 13.0
            bracket = "Inside Heavy Industrial Plant"
        elif dist_km <= 2.0:
            score = 10.0
            bracket = "Industrial Facility Buffer"
        elif dist_km <= 5.0:
            score = 7.0
            bracket = "Industrial Complex Vicinity"
        elif dist_km <= 15.0:
            score = 4.0
            bracket = "Regional Industrial Sector"
        else:
            score = 2.0
            bracket = "Remote Industrial Buffer"

    elif asset_type == "mining":
        if dist_km <= 1.0:
            score = 9.0
            bracket = "Active Mining Lease / Coal Pit"
        elif dist_km <= 5.0:
            score = 6.0
            bracket = "Mining Basin Vicinity"
        elif dist_km <= 15.0:
            score = 4.0
            bracket = "Regional Mining Belt"
        else:
            score = 2.0
            bracket = "Remote Buffer"

    elif asset_type == "forest_reserve":
        if dist_km <= 2.0:
            score = 4.0
            bracket = "Forest Reserve Core"
        else:
            score = 2.0
            bracket = "Wildland Interface"
    else:
        score = 2.0
        bracket = "Rural Agricultural Buffer"

    # Format detail string with true metric distance
    if dist_km < 1.0:
        dist_str = f"{int(round(dist_km * 1000))}m"
    else:
        dist_str = f"{dist_km:.2f} km" if dist_km < 10 else f"{dist_km:.1f} km"

    detail = f"{bracket} ({dist_str} to {clean_target}, {score:.1f}/{WEIGHT_INDUSTRIAL_PROX:.0f} pts)"
    return score, detail
...
def compute_incident_risk(incident: dict) -> dict:
    ...
    dist_km = incident.get("facility_distance_km")
    lat_val = incident.get("latitude")
    lon_val = incident.get("longitude")

    s_prox, desc_prox = calculate_proximity_score(
        dist_km=dist_km,
